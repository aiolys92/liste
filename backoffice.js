...
// Vérifier l'auth dès le chargement
...
const BO = {
  ...
  populateFilters() {
    ...
    sel('filterType', this.types); sel('filterCategory', this.categories);
    sel('filterPriority', this.priorities); sel('filterState', this.states);
  },
  ...
  populateFormSelects() {
    ...
    sel('fType', this.types); sel('fCategory', this.categories);
    sel('fPriority', this.priorities); sel('fState', this.states);
  },
  ...
  renderStats() {
    ...
    const newCount = t.filter(b => b.state === 'Nouveau').length;
    const critical = t.filter(b => b.priority === 'Critique').length;
    const progress = t.filter(b => b.state === 'En cours').length;
    const resolved = t.filter(b => b.state === 'Résolu').length;
    document.getElementById('statsRow').innerHTML = `
      <div class="stat-chip"><strong>${total}</strong> rapports</div>
      <div class="stat-chip" style="color:var(--s-new)"><span class="stat-chip-dot"></span><strong>${newCount}</strong> nouveaux</div>
  ...
  render() {
    ...
    if (page.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><span class="empty-icon">⊘</span><p>Aucun bug ne correspond aux filtres.</p></div></td></tr>`;
    } else {
  ...
  openCreate() {
    ...
    document.getElementById('fType').value     = this.types[0];
    document.getElementById('fCategory').value = this.categories[0];
    document.getElementById('fPriority').value = this.priorities[2];
  ...
  async save() {
    ...
    if (!title || !description || !date) {
      this.showNotif('Remplissez tous les champs obligatoires.', true); return;
    }
    ...
    try {
      ...
      this.showNotif('Erreur : ' + e.message, true);
    } finally {
  ...
  async confirmDelete() {
    ...
    const delBtn = document.querySelector('#confirmOverlay .btn-danger');
    delBtn.textContent = 'Suppression…'; delBtn.disabled = true;
    try {
      ...
      delBtn.textContent = 'Supprimer'; delBtn.disabled = false;
    }
  },
